...
abstract contract CalculatorBase is RatesTestBase {
    ...
    function testApproxMatch() public view {
        for (uint256 i = 0; i < 9899; i++) {
            ...
            assertTrue(
                calculatedRate >= mappingRate - tolerance && calculatedRate <= mappingRate + tolerance,
    ...
    function testRoundTrip() public {
        for (uint256 i = 0; i < 9899; i++) {
            ...
            emit log_string(
                string.concat(
                    "Input basis points: ",
                    vm.toString(basisPoints),
                    "\nRay rate: ",
                    vm.toString(rayRate),
                    "\nRound trip basis points: ",
                    vm.toString(roundTrip)
                )
            );
    ...
    function testPracticalDifferenceAcceptable() public {
        ...
        for (uint256 i = 0; i < 9899; i++) {
            ...
            uint256 diff = mappingDebt > calculatedDebt ? mappingDebt - calculatedDebt : calculatedDebt - mappingDebt;
            ...
            if (diff > 1e9) {
                emit log_string(
                    string.concat(
                        ...
                        " bps:\n",
                        "Mapping debt: ",
                        vm.toString(mappingDebt / 1e27),
                        ".",
                        vm.toString((mappingDebt % 1e27) / 1e18),
                        " units\n",
                        "Calculated debt: ",
                        vm.toString(calculatedDebt / 1e27),
                        ".",
                        vm.toString((calculatedDebt % 1e27) / 1e18),
                        " units\n",
                        "Difference: ",
                        vm.toString(diff / 1e18),
                        ".",
                        vm.toString(diff % 1e18),
                        " units"
                    )
